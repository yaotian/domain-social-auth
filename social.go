...
type SocialAuth struct {
	...
	providers          map[SocialType]Provider
	providersByPath    map[string]Provider
}

func (this *SocialAuth) RegisterProvider(prov Provider) error {
	if this.providers == nil {
		this.providers = make(map[SocialType]Provider)
	}

	if this.providersByPath == nil {
		this.providersByPath = make(map[string]Provider)
	}
	...
	path := prov.GetPath()
	if this.providersByPath[path] != nil {
		return fmt.Errorf("path `%s` is already in used", path)
	}
	this.providers[typ] = prov
	this.providersByPath[path] = prov
	return nil
}

// Get provider by SocialType
func (this *SocialAuth) GetProviderByType(typ SocialType) (Provider, bool) {
	if p, ok := this.providers[typ]; ok {
		return p, true
	} else {
		return nil, false
	}
...
func (this *SocialAuth) GetProviderByPath(path string) (Provider, bool) {
	if p, ok := this.providersByPath[path]; ok {
		return p, true
	} else {
		return nil, false
	}
...
func (this *SocialAuth) getProvider(ctx *context.Context) Provider {
	...
	p, ok := this.GetProviderByPath(path)
	if ok {
		return p
	}
...
func (this *SocialAuth) OAuthAccess(ctx *context.Context) (redirect string, userSocial *DomainUserSocial, failedErr error) {
	_, isLogin := this.app.IsUserLogin(ctx)
...
func (this *SocialAuth) ConnectAndLogin(ctx *context.Context, socialType SocialType, uid int) (string, *DomainUserSocial, error) {
	...
	var p Provider
	if p, _ = this.GetProviderByType(socialType); p == nil {
